...
def get_matched_shapes(shapes, target_halos, rep_key='rep',
    ...
    locations = get_locations(
        ...
        target_halos[rep_key])
    assert np.array_equal(target_halos['fof_halo_id'][locations],
                          shapes['fof_halo_tag']), "Fof tag arrays don't match"
    assert np.array_equal(target_halos[rep_key][locations],
                          shapes['replication']), "Replication mismatch"
    ...
    ordered_evecs = np.asarray([shapes[evectors][i].T[reorder[i]]
                               for i in range(nvals)])
    # select evector corresponding to largest evalue
    major_axis_evectors = ordered_evecs[:, 2]
    # check that normalization is correct
    norms = np.asarray([np.dot(major_axis_evectors[i],
                               major_axis_evectors[i].T) for i in range(nvals)])
