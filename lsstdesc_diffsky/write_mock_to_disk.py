...
def write_umachine_healpix_mock_to_disk(
    ...
    file_ids = [
        int(d) for d in re.findall(
            r'\d+',
            os.path.splitext(output_mock_basename)[0])]
    ...
    galaxy_id_offset = int(cutout_number_true*cutout_id_offset +
                           z_offsets_not_im[str(Nside)][z_range_id])
    ...
    print('Writing catalog version number {}.{}.{}'.format(versionMajor, versionMinor,
                                                           versionMinorMinor))
    ...
    print("\nComputing ssp tables for {} z values: {:.2f} < z < {:.2f} (dz={:.2f})".format(n_z_table,
                                                                                           z_min, z_max, dz))
    ssp_restmag_table = precompute_ssp_restmags(
        ssp_wave, ssp_flux, filter_waves, filter_trans)
    ...
    for key in model_keys:
        ...
        model_req = [k for k in SED_params['filter_keys']
                     if filt_req in k and band_req in k]
    ...
    for a, b, c in gen:
        ...
        target_halos = get_astropy_table(
            healpix_data[snapshot],
            halo_unique_id=halo_unique_id)
        fof_halo_mass_max = max(
            np.max(
                target_halos[fof_halo_mass].quantity.value),
            fof_halo_mass_max)
        ...
        target_halos['axis_A_length'] = 1.5 * \
            spherical_halo_radius  # crude fix for B and C shrinking
        ...
        axis_A = normalized_vectors(random_vectors) * \
            target_halos['axis_A_length'].reshape((-1, 1))
        ...
        shapes = get_halo_shapes(snapshot, target_halos['fof_halo_id'], target_halos['lightcone_replication'],
                                 shape_dir)
        if shapes:
        ...
        assert (len(np.unique(source_halo_ids)) == len(
            source_halo_ids)), "Source halo ids not unique"
        ...
        gs_results = galsample(source_galaxies_host_halo_id,
                               source_halo_ids, target_halo_ids,
                               [noisy_log_src_mass], [log_target_mass])
        ...
        if not synthetic_params['skip_synthetics']:
            mock, synthetic_dict = add_low_mass_synthetic_galaxies(mock, seed, synthetic_halo_minimum_mass,
                                                                   ...
                                                                   previous_redshift, Vtotal, cosmology,
                                                                   mstar_min)
        else:
            synthetic_dict = {}
        ...
        galaxy_id_bound = cutout_number*cutout_id_offset + \
        ...
        print('...Saved {} galaxies to dict'.format(
    ...
    if len(output_mock) > 0:
        ...
        write_output_mock_to_disk(output_color_mock_fname, output_mock, commit_hash, seed,
                                  synthetic_halo_minimum_mass, cutout_number_true, Nside,
                                  cosmology,
    ...
    print('Maximum halo mass for {} ={}\n'.format(
        output_mock_basename, fof_halo_mass_max))
...
def get_volume_factor(redshift, previous_redshift, Vtotal, cosmology):
    ...
    Vshell = cosmology.comoving_volume(
        float(redshift)).value - cosmology.comoving_volume(float(previous_redshift)).value
...
def get_astropy_table(table_data, halo_unique_id=0, check=False, cosmology=None):
    ...
    t['halo_id'] = (np.arange(len(table_data['id'])) *
                    halo_id_offset + halo_unique_id).astype(int)
...
def add_low_mass_synthetic_galaxies(mock, seed, synthetic_halo_minimum_mass, redshift, synthetic_number,
                                    randomize_redshift_synthetic,
                                    previous_redshift, Vtotal, cosmology, mstar_min,
                                    use_substeps_synthetic=False, nzdivs=6,
                                    ):
    ...
    print(".......changed {}/{} M* values; max/min new log(M*) {:.2f}/{:.2f}".format(np.count_nonzero(~diff), len(diff),
                                                                                     np.max(
                                                                                         np.log10(new_mstar_real[~diff])),
...
def build_output_snapshot_mock(
    ...
    dc2['target_halo_fof_halo_id'] = 0.
    dc2['target_halo_redshift'] = 0.
    dc2['target_halo_x'] = 0.
    dc2['target_halo_y'] = 0.
    dc2['target_halo_z'] = 0.
    ...
    source_galaxy_halo_keys = ('mp', 'vmp', 'rvir', 'upid', 'host_rvir', 'host_mp', 'host_rvir',
                               'halo_id', 'has_fit', 'is_main_branch')
    source_galaxy_prop_keys = ('host_dx', 'host_dy', 'host_dz',
                               'host_dvx', 'host_dvy', 'host_dvz',
                               'obs_sm', 'obs_sfr')
    ...
    source_galaxy_keys = source_galaxy_halo_keys + \
        source_galaxy_prop_keys + tuple(SFH_param_keys)
    ...
    ultra_high_mvir_halo_mask = (
        dc2['source_galaxy_upid'] == -
        1) & (
        dc2['target_halo_mass'] > max_umachine_halo_mass)
    num_to_remap = np.count_nonzero(ultra_high_mvir_halo_mask)
    if num_to_remap > 0:
        print(
            ".....remapping stellar mass of {0} BCGs in ultra-massive halos".format(num_to_remap))
        ...
        assert np.shape(halo_mass_array) == (
            num_to_remap, ), "halo_mass_array has shape = {0}".format(
            np.shape(halo_mass_array))
        assert np.shape(mstar_array) == (
            num_to_remap, ), "mstar_array has shape = {0}".format(
            np.shape(mstar_array))
        assert np.shape(redshift_array) == (
            num_to_remap, ), "redshift_array has shape = {0}".format(
            np.shape(redshift_array))
        assert np.shape(upid_array) == (
            num_to_remap, ), "upid_array has shape = {0}".format(
            np.shape(upid_array))
        assert np.all(
            mhalo_ratio >= 1), "Bookkeeping error: all values of mhalo_ratio ={0} should be >= 1".format(mhalo_ratio)
    ...
    if nsats > 0:
        ...
        b_to_a = dc2['target_halo_axis_B_length'][satmask] / \
            dc2['target_halo_axis_A_length'][satmask]
        c_to_a = dc2['target_halo_axis_C_length'][satmask] / \
            dc2['target_halo_axis_A_length'][satmask]
    ...
    print('.....number of galaxies before adding synthetic satellites = {}'.format(
        len(dc2['source_galaxy_halo_id'])))

    # add synthetic cluster galaxies
    fake_cluster_sats = model_synthetic_cluster_satellites(dc2, Lbox=0., host_conc=host_conc, SFH_keys=list(SFH_param_keys),
    ...
    if len(fake_cluster_sats) > 0:
        ...
        print('.....time to create {} galaxies in fake_cluster_sats = {:.2f} secs'.format(len(fake_cluster_sats['target_halo_id']),
                                                                                          time()-check_time))
    else:
    ...
    if synthetic_dict and len(synthetic_dict['mp']) > 0:
        ...
        if len(lowmass_mock) > 0:
            ...
            print('.....time to create {} galaxies in synthetic_lowmass_mock = {:.2f} secs'.format(len(lowmass_mock['target_halo_id']),
                                                                                                   time()-check_time))
    ...
    if SED_params['ellipticity_model_mag']:
        ...
        dc2['diskEllipticity1'] = np.array(np.cos(2.0*pos_angle)*ellip_disk, dtype='f4')
        dc2['diskEllipticity2'] = np.array(np.sin(2.0*pos_angle)*ellip_disk, dtype='f4')
        dc2['positionAngle'] = np.array(pos_angle*180.0/np.pi, dtype='f4')
        if 'bulge_to_total_ratio' in dc2.colnames:
            tot_ellip = (1. - dc2['bulge_to_total_ratio']) * \
                ellip_disk + dc2['bulge_to_total_ratio']*ellip_spheroid
    ...
    # Galaxy ids
    dc2['galaxy_id'] = np.arange(galaxy_id_offset,
                                 galaxy_id_offset + len(dc2['target_halo_id'])).astype(int)
    print(
        '\n.....Min and max galaxy_id = {} -> {}'.format(
            np.min(
                dc2['galaxy_id']), np.max(
                dc2['galaxy_id'])))
...
def generate_SEDs(dc2, SED_params, cosmology, w0, wa,
    ...
    if nmissed > 0 or (nmissed < 0 and nfail > 0) or (
        ...
        mc_parlist = [mc_mah_params] if use_diffmah_pop else [
            mc_mah_params, mc_ms_u_params, mc_q_u_params]
        for key_label, mc_params in zip(key_labels, mc_parlist):
            for i, key in enumerate(
                    SED_params[key_label]):  # potential bug here if some other subset of fit params selected
                dc2[key][failed_mask] = mc_params[:, i]
                print('.......saving pop model parameters {}'.format(key))

    params = {}
    _res = get_params(dc2, mah_keys=SED_params[mah_keys],
                      ms_keys=SED_params[ms_keys], q_keys=SED_params[q_keys])
    ...
    if SED_params['dust']:
        ...
        attenuation_factors = precompute_dust_attenuation(SED_params['filter_waves'],
                                                          SED_params['filter_trans'],
                                                          dc2['redshift'], dust_params)
...
def get_sky_coords(dc2, cosmology, redshift_method='halo', Nzgrid=50):
    ...
    if redshift_method is not None:
        print('\n.....Generating lightcone redshifts using {} method'.format(redshift_method))
...
def get_skyarea(output_mock, Nside):
    ...
    if Nside > 8:
        ...
        if np.isclose(skyarea, nominal_skyarea, rtol=.02):
            print(' Replacing calculated sky area {} with nominal_area'.format(skyarea))
            skyarea = nominal_skyarea
        if np.isclose(skyarea, nominal_skyarea/2.,
                      rtol=.01):  # check for half-filled pixels
...
def write_output_mock_to_disk(output_color_mock_fname, output_mock, commit_hash, seed,
                              synthetic_halo_minimum_mass, cutout_number, Nside, cosmology,
