...
def calc_rest_sed_disk_bulge_knot_singlegal(
    ...
    bulge_weights = bulge_age_weights.reshape((n_age, 1))
    dd_weights = dd_age_weights.reshape((n_age, 1))
    knot_weights = knot_age_weights.reshape((n_age, 1))

    rest_sed_bulge_nodust = (
        jnp.sum(bulge_weights * ssp_data.ssp_flux, axis=(0,)) * mbulge
    )
    rest_sed_dd_nodust = jnp.sum(dd_weights * ssp_data.ssp_flux, axis=(0,)) * mdd
    ...
    rest_sed_bulge = (
        jnp.sum(bulge_weights * ssp_data.ssp_flux * frac_dust_trans, axis=(0,)) * mbulge
    )

    rest_sed_dd = (
        jnp.sum(dd_weights * ssp_data.ssp_flux * frac_dust_trans, axis=(0,)) * mdd
    )
    rest_sed_knot = (
        jnp.sum(knot_weights * ssp_data.ssp_flux * frac_dust_trans, axis=(0,)) * mknot
    )
