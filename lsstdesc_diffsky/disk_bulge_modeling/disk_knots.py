...
@jjit
def _disk_knot_kern(tarr, tobs, sfh, sfh_disk, fburst, fknot, age_weights_burst, ssp_lg_age_gyr):
    ...
    age_weights_dd = calc_age_weights_from_sfh_table(tarr, sfh_diffuse_disk, ssp_lg_age_gyr, tobs)
    ...
    burst_knot_age_weights = mburst_by_mknot * age_weights_burst + (1 - mburst_by_mknot) * age_weights_dd
    age_weights_knot = jnp.where(mburst_by_mknot > 1, age_weights_burst, burst_knot_age_weights)

    mburst_dd = jnp.where(mburst_by_mknot > 1, mburst - mknot, 0.0)

    mdd_tot = mdd + mburst_dd
    age_weights_dd = (mdd / mdd_tot) * age_weights_dd + (mburst_dd / mdd_tot) * age_weights_burst
