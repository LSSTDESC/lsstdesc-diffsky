...
def synthetic_logmpeak(mpeak_orig, desired_logm_completeness=default_desired_logm_completeness,
    ...
    if np.count_nonzero(zero_mask) > 3:
        c1, c0 = np.polyfit(
            logmmids_fit[zero_mask], np.log10(
                counts_fit[zero_mask]), deg=1)

    logmbins_extrap = np.arange(
        desired_logm_completeness -
        dlogm_fit,
        highm_fit +
        dlogm_fit,
        dlogm_fit)
    ...
    delta_counts_cumprob = np.cumsum(
        delta_counts_extrap)/float(delta_counts_extrap.sum()+1)
